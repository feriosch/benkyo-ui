...
export class EditWordFormComponent implements OnInit {
  ...
  initializeForm(): void {
    this.editWordForm = new FormGroup({
      ...
      spanish: new FormControl(this.originalWord?.spanish, [
        Validators.required,
      ]),
      type: new FormGroup({
        noun: new FormControl(this.originalWord?.type.noun, [
          Validators.required,
        ]),
        suruVerb: new FormControl(this.originalWord?.type.suru_verb, [
          Validators.required,
        ]),
        noAdjective: new FormControl(this.originalWord?.type.no_adjective, [
          Validators.required,
        ]),
        naAdjective: new FormControl(this.originalWord?.type.na_adjective, [
          Validators.required,
        ]),
        iAdjective: new FormControl(this.originalWord?.type.i_adjective, [
          Validators.required,
        ]),
        adverb: new FormControl(this.originalWord?.type.adverb, [
          Validators.required,
        ]),
        verb: new FormControl(this.originalWord?.type.verb, [
          Validators.required,
        ]),
        adjectivalNoun: new FormControl(
          this.originalWord?.type.adjectival_noun,
          [Validators.required]
        ),
        adverbialNoun: new FormControl(this.originalWord?.type.adverbial_noun, [
          Validators.required,
        ]),
        counter: new FormControl(this.originalWord?.type.counter, [
          Validators.required,
        ]),
      }),
      tags: new FormGroup({
        ateji: new FormControl(this.originalTags.includes('ateji'), [
          Validators.required,
        ]),
        common: new FormControl(this.originalTags.includes('common'), [
          Validators.required,
        ]),
        expression: new FormControl(this.originalTags.includes('expression'), [
          Validators.required,
        ]),
        honorific: new FormControl(this.originalTags.includes('honorific'), [
          Validators.required,
        ]),
        humble: new FormControl(this.originalTags.includes('humble'), [
          Validators.required,
        ]),
        intransitive: new FormControl(
          this.originalTags.includes('intransitive'),
          [Validators.required]
        ),
        jlptN1: new FormControl(this.originalTags.includes('jlpt_n1'), [
          Validators.required,
        ]),
        notJoyo: new FormControl(this.originalTags.includes('joyogai'), [
          Validators.required,
        ]),
        onomatopoeic: new FormControl(
          this.originalTags.includes('onomatopeic'),
          [Validators.required]
        ),
        transitive: new FormControl(this.originalTags.includes('transitive'), [
          Validators.required,
        ]),
        usuallyKana: new FormControl(
          this.originalTags.includes('usually_kana'),
          [Validators.required]
        ),
      }),
    ...
    this.vocabularyService.getWordById(this.id).subscribe((response) => {
      ...
      if (this.originalWord?.sentences) {
        this.originalWord?.sentences.forEach((sentence: Sentence) => {
          this.sentencesControl.push(
            new FormGroup({
              japanese: new FormControl(sentence.sentence, [
                Validators.required,
              ]),
